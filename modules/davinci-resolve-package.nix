# Vendored copy of nixpkgs' pkgs/by-name/da/davinci-resolve/package.nix
# (as of nixos-unstable commit b1b875982b17dabde9b4a37f3e229e74913e6db3,
# 2026-09), with exactly one line changed: the non-studio outputHash.
#
# This package fetches its actual installer dynamically at build time
# from Blackmagic's own download API -- there's no stable, redistributable
# URL, since it's proprietary software gated behind a (fake, auto-filled)
# registration form. nixpkgs pins a hash for whatever that endpoint
# returned when the package was last updated, but Blackmagic periodically
# replaces the file behind the same "DaVinci Resolve 21.1" version string
# without bumping anything nixpkgs tracks -- confirmed this is an
# established, recurring maintenance pattern for this specific package
# (nixpkgs even ships its own dedicated update-davinci-resolve script for
# it). Confirmed nixos-unstable's current tip still has the stale hash --
# not yet fixed upstream, so this isn't a local misconfiguration.
#
# A plain `pkgs.davinci-resolve.overrideAttrs` can't fix this: the actual
# downloadable package builds an inner `davinci` derivation (holding the
# real src fetch) and wraps it in an FHS environment whose build script
# already has that inner derivation's store path baked into it as a plain
# interpolated string at parse time -- overriding the outer package's
# `src`, or even the `davinci` passthru attribute, doesn't propagate into
# what the FHS wrapper's own builder script actually references. Vendoring
# the whole file (self-contained, no sibling files in the original
# directory) with just the one corrected hash is the direct fix.
#
# The corrected hash below is exactly what Nix's own error output
# reported as the real, current hash ("got: ...") when the stale one
# failed -- not guessed.
#
# TODO: once nixpkgs' own pinned hash catches up to match this (check via
# `nix eval github:NixOS/nixpkgs/nixos-unstable#davinci-resolve.davinci.
# src.outputHash`), delete this file and the overlay pulling it in
# (modules/users.nix), and go back to plain `pkgs.davinci-resolve`.
{
  stdenv,
  lib,
  cacert,
